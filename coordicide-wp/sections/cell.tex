...
%On the contrary case, it would prefer other malicious nodes and effectively render the “proof” useless.
%If, on the other hand, the neighbors are verifiably random -- which needs to be achieved by modification in the peering system -- the probability that a sufficiently large \enquote{island} of malicious nodes arises can be controlled with the parameter~$k$.
...
%		\includegraphics[width=\linewidth]{ca1}
%		\caption{two conflicting transactions are issued}
%	\end{subfigure}
%	\hfill
%	\begin{subfigure}[b]{0.32\linewidth}
%		\includegraphics[width=\linewidth]{ca2}
%		\caption{nodes form their initial opinions}
%	\end{subfigure}
	\hfill
